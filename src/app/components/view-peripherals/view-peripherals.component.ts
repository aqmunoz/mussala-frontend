...
export class ViewPeripheralsComponent implements OnInit {
  ...
  confirmDelete: string = 'Delete peripheral';
  ...
  deletePeripheral(index: number, e: { _id: any; }){
  ...
  handleAction(valueEmitted: any) {
    if (valueEmitted.accept) {
      this.deletePeripheral(valueEmitted.params._id, valueEmitted.params.element)
    }
  }
